...
<?php
    $conn=mysqli_connect("sql306.byethost.com","b4_25247848","lovelyfriend143","b4_25247848_cspl3");
	session_start();
	error_reporting(0);
	...
	{
		$query="INSERT into matches(`t1`,`t1s`,`t2`,`t2s`) VALUES ('".$_POST['t1']."','0,0,0','".$_POST['t2']."','0,0,0')";
		if(mysqli_query($conn,$query))
		{
			echo "Match updated";
		}
	...
	if(isset($_POST['submit5']))
	{
		...
		if($update[0]!=$res[0])
		{
			...
			if(!mysqli_query($conn,"UPDATE teams set matches='".$update2[0]."',lost='".$update2[1]."' where id='".$update[1]."'"))
			{
				echo "error";
			}
			...
			if(!mysqli_query($conn,"UPDATE teams set matches='".$update2[0]."',points='".$update2[1]."' where id='".$update[0]."'"))
			{
				echo "error";
			}
			$query="UPDATE matches set status=-1,changed=1 where id='$mid'";	
		}
		else
		{
			$query="UPDATE matches set status=-1 where id='$mid'";	
		}
		mysqli_query($conn,$query);
		if(!mysqli_query($conn,$query))
		{
	...
	if(isset($_POST['submit6']))
	{
		...
		{
			$per=explode(",",$performed);
			if(sizeof($per)!=3)
			{
                $query="UPDATE matches set '$ts'=".$performed."' where id='$mid'";
                if(!mysqli_query($conn,$query))
                {
                    echo "Error";
                }
				...
				{
					$performed=$performed.",0,1";
				}
				else
				{
					$performed=$performed.",0,0";
				}
			}
			$query="INSERT into scorecard(mid,tid,pid,name,score,state,innings) VALUES ('$mid','$tid','$pid','$name','$performed','$action','$ini')";
			mysqli_query($conn,$query);
		}
	}
